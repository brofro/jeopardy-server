...
db_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "jeopardy.db"))
...
async def get_round(round_value: int, category: str = None):
    if round_value not in [1, 2]:
        raise HTTPException(status_code=400, detail="Round value must be 1 (Single Jeopardy) or 2 (Double Jeopardy)")
    ...
    try:
        ...
        try:
            ...
            stmt = (
                ...
                .limit(6 if not category else 5)  # Get 5 if we need to add a specific category
            )
            ...
            if not categories:
                ...
                raise HTTPException(status_code=404, detail="No categories found for this round")
            ...
            for category in categories:
                ...
                if len(clues) != 5:
                    ...
                    logger.error(f"Could not find 5 clues for category {category} with matching air date")
                ...
                for clue in clues:
                    ...
                    clues_list.append({
                        ...
                        "air_date": clue.air_date.isoformat() if clue.air_date else None,
                        "notes": clue.notes
                    })
...
async def submit_answer(request: Request):
    try:
        ...
        if not body.get("clue_id") or not body.get("user_answer"):
            raise HTTPException(status_code=400, detail="Both clue_id and user_answer are required")
        ...
        try:
            ...
            similarity = fuzz.partial_token_set_ratio(user_answer.lower(), correct_answer.lower())
